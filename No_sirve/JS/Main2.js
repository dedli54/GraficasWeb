...
//Declaration Global Variables for the 3D model
let arbol;
let hongo;
...
//imagen de fondo
const skytexture = fondoTexture.load('IMAGENES/CieloTextura2.jpg');
...
//Luz Ambiental
const ambientLight = new THREE.AmbientLight(0xFFA500, 0.3); // (color, intensity)
scene.add(ambientLight);

// Iluminacion Focal
const spotLight1 = new THREE.SpotLight(0xffffff, 3); // Increase intensity
spotLight1.position.set(-220, 300, 200); // Position the spotlight
spotLight1.angle = Math.PI / 8; // Sharper cone angle
spotLight1.penumbra = 0.2;
spotLight1.decay = 2;
spotLight1.distance = 600;

const spotLight2 = new THREE.SpotLight(0xffffff, 3); // Increase intensity
spotLight2.position.set(-200, 300, -200); // Position the spotlight
spotLight2.angle = Math.PI / 8; // Sharper cone angle
...
// Add target objects to the spotlights
spotLight1.target = new THREE.Object3D();
spotLight1.target.position.set(-330, 9, -90);
scene.add(spotLight1.target);

spotLight2.target = new THREE.Object3D();
spotLight2.target.position.set(-330, 9, -90);
...
function createTerrain() {
  ...
  const texture = new THREE.TextureLoader()
    .setPath("")
    .load("IMAGENES/MapaTextura2.jpg");
...
const sliders = {
  widthseg: 160,
  heightseg: 160,
  heightMap: "IMAGENES/MapaAlturas2.jpg",
  hortexture: 1,
  vertTexture: 1,
  dispScale: 120
};
...
// Variables globales para el control del mouse
...
// Función para inicializar el control del mouse
function initMouseControl() {
  // Agregar eventos de mouse para el control de la cámara
...
// Función para manejar el evento de liberación del mouse
function onMouseUp() {
  isDragging = false;
}
...
function onMouseMove(event) {
  if (isDragging) {
    ...
    // Ajustar la orientación de la cámara en función del movimiento del mouse
    camera.rotation.y -= deltaMousePosition.x * 0.5; // Ajustar la rotación horizontal
...
// Llamar a la función para inicializar el control del mouse
initMouseControl();
...
const reduccionVida = 1; // Velocidad de reducción de la vida del robot
const dañarEnemigo = 10;
const laserfatal = 3;
const bombafatal = 6;
...
function disparar() {
  ...
  function moverProyectil() {
      ...
      }else if ( proyectilBoundingBox.intersectsBox(enemy1BoundingBox)) {
      ...
      if (enemyVida2 === 0) {
        ...
        // Aquí puedes agregar la lógica para lo que quieras hacer cuando el robot sea destruido
      }else if (proyectilBoundingBox.intersectsBox(enemy2BoundingBox)) {
        // Reducir la vida del robot si hay colisión
      ...
      if (enemyVida3 === 0) {
        ...
        // Aquí puedes agregar la lógica para lo que quieras hacer cuando el robot sea destruido
      }else if (proyectilBoundingBox.intersectsBox(enemy3BoundingBox)) {
        // Reducir la vida del robot si hay colisión
      ...
      if (enemyVida4 === 0) {
        ...
        // Aquí puedes agregar la lógica para lo que quieras hacer cuando el robot sea destruido
      }else if (enemyVida4!= 0 && proyectilBoundingBox.intersectsBox(enemy4BoundingBox)) {
        // Reducir la vida del robot si hay colisión
      ...
      if (enemyVida5 === 0) {
        ...
        // Aquí puedes agregar la lógica para lo que quieras hacer cuando el robot sea destruido
      }else if (enemyVida5!= 0 && proyectilBoundingBox.intersectsBox(enemy5BoundingBox)) {
        // Reducir la vida del robot si hay colisión
    ...
    function (gltf) {
      ...
      function cargarPlataforma() {
        ...
        function animate() {
            ...
            model.position.y = 0; // Ajustar la altura del modelo si es necesario
            ...
            if (robotBoundingBox && enemy1BoundingBox&& enemy2BoundingBox && moneyBoundingBox && ammoBoundingBox && ammo2BoundingBox&& ammo3BoundingBox
              ...
              &&door2BoundingBox&&cab1BoundingBox&&cab2BoundingBox&&arbolBoundingBox&&hongoBoundingBox&& mixer) {
                robotBoundingBox.setFromObject(model);
        ...
        document.addEventListener('keydown', (event) => {
          previousPosition.copy(model.position);
            switch (event.key) {
                case 'w':
                    model.position.z -= cameraSpeed; // adelante
                    if (handleCollisions()) model.position.copy(previousPosition);
                    break;
                case 'a':
                    model.position.x -= cameraSpeed; // izquierda
                    if (handleCollisions()) model.position.copy(previousPosition);
                    break;
                case 's':
                    model.position.z += cameraSpeed; // atrás
                    if (handleCollisions()) model.position.copy(previousPosition);
                    break;
                case 'd':
                    model.position.x += cameraSpeed; // derecha
                    if (handleCollisions()) model.position.copy(previousPosition);
                    break;
                case 'x':
                    model.rotation.y -= 0.2; // derecha
                    camera.rotation.y -= cameraRotationSpeed;
                    break;
                case 'z':
                    model.rotation.y += 0.2; // izquierda
                    camera.rotation.y += cameraRotationSpeed;
                    break;
                case 'e':
                      if (robotMunicion > 0) {
...
loaderGLTF.load(
    ...
    function (gltf) {
        ...
        base1.position.set(-130, -0.5, 200); 
...
loaderGLTF.load(
  ...
  function (modelin) {
    ...
    built1.position.set(-150, 0, 150);
    scene.add(built1);

    built2 = modelin.scene.clone();
    built2 .scale.set(0.4, 0.4, 0.4);
    built2 .position.set(170, 0, -50);
    scene.add(built2);
...
loaderGLTF.load(
  ...
  function (modelin) {
    ...
    door1.position.set(170, 0, -22);
    scene.add(door1);

    door2 = modelin.scene.clone();
    door2.scale.set(1.8, 1.8, 1,8);
    door2.position.set(-145, 0, 140);
    scene.add(door2);
...
let cab1, cab2;
  loaderGLTF.load(
    ...
    function (modelin) {
      ...
      cab1.position.set(200, 0.1, -35);
      scene.add(cab1);

      cab2 = modelin.scene.clone();
      cab2.scale.set(4, 4, 4);
      cab2.position.set(-170, 0.1, 160);
      scene.add(cab2);
  ...
  loaderGLTF.load(
    ...
    function (modelin) {
      ...
      medicina.position.set(-150, 4, 30);
  ...
  loaderGLTF.load(
    ...
    function (modelin) {
      ...
      modeloo1.position.set(-30, 5.5, 60);
        ...
        const puntoA = new THREE.Vector3(-150, 5.5, 10);
        const puntoB = new THREE.Vector3(-330, 9, -90);
...
loaderGLTF.load(
  ...
  function (modelin) {
    modeloo2 = modelin.scene;
      ...
      // Definir los puntos A y B de la ruta
      const puntoA = new THREE.Vector3(-80, 5.5, 0);
      const puntoB = new THREE.Vector3(-330, 9, -90);
...
loaderGLTF.load(
  ...
  function (modelin) {
    modeloo3 = modelin.scene;
      ...
      // Definir los puntos A y B de la ruta
      const puntoA = new THREE.Vector3(-100, 5.5, -10);
      const puntoB = new THREE.Vector3(-330, 9, -90);
...
loaderGLTF.load(
  ...
  function (modelin) {
    modeloo4 = modelin.scene;
      ...
      // Definir los puntos A y B de la ruta
      const puntoA = new THREE.Vector3(-120, 5.5, 0);
      const puntoB = new THREE.Vector3(-330, 9, -90);
...
loaderGLTF.load(
  ...
  function (modelin) {
    modeloo5 = modelin.scene;
      ...
      // Definir los puntos A y B de la ruta
      const puntoA = new THREE.Vector3(-110, 5.5, -20);
      const puntoB = new THREE.Vector3(-330, 9, -90);
